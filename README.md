...
A few entries fall outside ISO: the EU uses its reserved `eu`, England uses the subdivision code `gb-eng`, and Mars and Tobago have no code (just `mars.svg`, `tobago.svg`). See the [full list](#coverage) for every code.
